...
show_system_info() {
    ...
    if command -v df &> /dev/null; then
        df -h / | grep -v "Filesystem" | awk '{print "根分区: 总空间 " $2 ", 已用 " $3 " (" $5 "), 可用 " $4}'
    else
        echo "无法获取磁盘信息"
    fi
    ...
    if command -v ip &> /dev/null; then
        ip -4 addr show | grep -v "127.0.0.1" | grep "inet " | head -n 2 | awk '{print "IP地址: " $2}' | cut -d/ -f1
    elif command -v ifconfig &> /dev/null; then
        ifconfig | grep "inet " | grep -v "127.0.0.1" | head -n 2 | awk '{print "IP地址: " $2}'
    else
        echo "无法获取网络信息"
    fi
    ...
    # GLIBC版本
    echo -e "\n${BOLD}GLIBC版本:${NC}"
    ldd --version | head -n 1 | grep -oE '[0-9]+\.[0-9]+$' || echo "无法获取GLIBC版本"
    
    # Docker信息
    echo -e "\n${BOLD}Docker信息:${NC}"
    if command -v docker &> /dev/null; then
        docker --version
        echo "Docker镜像数: $(docker images -q | wc -l)"
    else
        echo "Docker未安装"
    fi
